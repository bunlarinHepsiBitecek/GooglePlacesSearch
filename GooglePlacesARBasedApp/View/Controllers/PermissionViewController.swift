...
class PermissionViewController: UIViewController {
    ...
    lazy var imageView: UIImageView = {
        ...
        temp.image = UIImage(named: "ny_empire_state_alone_blurred.jpg")
        return temp
    }()
    ...
    lazy var centerViewTopImage: UIImageView = {
        ...
        temp.clipsToBounds = true
        temp.isUserInteractionEnabled = false
        temp.image = UIImage(named: "ny_empire_state.jpg")
...
//        temp.clipsToBounds = true
        temp.isUserInteractionEnabled = false
        temp.image = UIImage(named: "camera.jpg")
        temp.contentMode = .scaleAspectFill
        temp.layer.cornerRadius = 47
        temp.layer.shadowColor = #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1)
        temp.layer.shadowOffset = CGSize(width: 0, height: 5)
    ...
    let mainSubject: UILabel = {
        ...
        label.font = UIFont(name: "Avenir-Medium", size: 26)
    ...
    lazy var stackViewForProcessButtons: UIStackView = {
        ...
        let temp = UIStackView(arrangedSubviews: [locationPermissionButton, cameraPermissionButton])
        temp.translatesAutoresizingMaskIntoConstraints = false
        temp.isUserInteractionEnabled = true
        temp.spacing = 10
        temp.alignment = .fill
    ...
    lazy var cameraPermissionButton: PermissionButtonView = {
        let temp = PermissionManager.shared.createPermissionButtonView(permissionType: .camera)
        //temp.isUserInteractionEnabled = false
        let tapGesture = UITapGestureRecognizer(target: self, action: Selector.pressedCameraButton)
        tapGesture.delegate = self
        temp.addGestureRecognizer(tapGesture)
        return temp
    }()
...
extension PermissionViewController {
    ...
    fileprivate func addViews() {
        ...
        self.centerView.addSubview(cameraViewContainer)
        self.centerView.addSubview(locationViewContainer)
        self.cameraViewContainer.addSubview(cameraImageView)
        ...
        NSLayoutConstraint.activate([
            ...
            centerViewContainer.heightAnchor.constraint(equalToConstant: 500),
            ...
            cameraViewContainer.leadingAnchor.constraint(equalTo: self.centerView.leadingAnchor, constant: 16),
            cameraViewContainer.topAnchor.constraint(equalTo: self.centerView.topAnchor, constant: 102),
            cameraViewContainer.heightAnchor.constraint(equalToConstant: 100),
            cameraViewContainer.widthAnchor.constraint(equalToConstant: 100),
            
            cameraImageView.centerXAnchor.constraint(equalTo: self.cameraViewContainer.centerXAnchor),
            cameraImageView.centerYAnchor.constraint(equalTo: self.cameraViewContainer.centerYAnchor),
            cameraImageView.heightAnchor.constraint(equalToConstant: 94),
            cameraImageView.widthAnchor.constraint(equalToConstant: 94),
            
            locationViewContainer.trailingAnchor.constraint(equalTo: self.centerView.trailingAnchor, constant: -16),
            locationViewContainer.topAnchor.constraint(equalTo: self.centerView.topAnchor, constant: 72),
            ...
            mainStackView.topAnchor.constraint(equalTo: self.centerViewTopImage.bottomAnchor, constant: 70),
            ...
            mainStackView.heightAnchor.constraint(equalToConstant: 240),
...
extension PermissionViewController: UIGestureRecognizerDelegate {
    ...
    @objc fileprivate func pressCameraButtonView(_ sender: UITapGestureRecognizer) {
        print("\(#function) pressed")
        startAnimationCommon(inputObject: cameraPermissionButton)
        
        PermissionManager.shared.requestPermission(permissionType: .camera, permissionResult: cameraPermissionButton.getPermissionResult())
        ...
        PermissionManager.shared.completionHandlerForLocationAuthorizationDidChange = { (updatedPermissionButtonProperty) -> Void in
            ...
            DispatchQueue.main.async {
                ...
                UIView.animate(withDuration: 0.3, animations: {
                    self.cameraPermissionButton.updateButtonView(permissionButtonProperty: updatedPermissionButtonProperty)
                }, completion: { (finish) in
...
fileprivate extension Selector {
    ...
    static let pressedCameraButton = #selector(PermissionViewController.pressCameraButtonView)
}
